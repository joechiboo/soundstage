...
**4a. 加測試使用者**：在「測試使用者」區塊把你自己的 Gmail 加進去。

> ⚠️ 漏掉這步的話，步驟 7 授權時會直接被擋，而錯誤訊息不會告訴你原因是這個。

**4b. 發布應用程式**：找到「發布狀態」→ 點「**發布應用程式**」→ 確認。

**為什麼一定要做 4b**：停在「測試中」狀態時，Google 規定 refresh token
**7 天後失效**，等於每週都要重新授權一次，自動化免談。改成正式版就沒這個限制。

會不會需要 Google 驗證？`youtube.upload` 是敏感範圍，要給一般大眾用的確要送
驗證。但**自用不需要** —— 發布後你自己照樣能授權，只是每次會看到一個「未驗證」
的警告畫面，點進階就能繼續。左側那個「驗證中心」是送審用的，你不用管。
...
| 症狀 | 原因 |
|---|---|
| 授權時被擋，說沒有權限 | 步驟 4a 的**測試使用者**沒加自己；或 4b 沒發布 |
| 每週都要重新授權 | 發布狀態還停在「測試中」，回步驟 4b |
| `找不到 OAuth client secret` | 步驟 6 的路徑或檔名不對，跑 `auth --status` 看它期待的位置 |
| `缺少 YouTube 上傳所需的套件` | 沒跑 `uv sync --extra upload` |
| `run_local_server` 卡住或失敗 | 步驟 5 的應用程式類型選錯，必須是**桌面應用程式** |
